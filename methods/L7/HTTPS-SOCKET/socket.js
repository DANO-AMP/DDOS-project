...
console.log('You need proxy, "proxy.txt"');
process.exit();
}
...
var parsed = url.parse(objetive);
    ...
    console.log('WHAT??? MADE BY @EMPFAKED YOU SKID ???');
    process.exit();
}
const sigalgs = [
...
var UAs = fs.readFileSync('ua.txt', 'utf-8').replace(/\r/g, '').split('\n');
     ...
     console.log('You need file ua.txt')
 }
class TlsBuilder {
    constructor (socket){
    ...
    http2TUNNEL(socket){
        ...
        payload['Accept-Language'] = 'utf-8;ru-RU,en-US,ru;en;q=0.9,q=0.8,q=0.7,q=0.6, iso-8859-1;q=0.5, *;q=0.1'
